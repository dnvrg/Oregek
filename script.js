...
async function analyzeImageWithGemini(imageData, patientId) {
    ...
    const prompt = 'Analyze the handwritten shopping list in the image. Identify each item and its corresponding quantity. The list may contain medications with foreign names (e.g., Tensiomin, Tanydon, Coverex). Transcribe these names exactly as they are written, without trying to correct them. For quantities, use standard Hungarian abbreviations (e.g., kilogramm should be kg, liter should be l, dekagramm should be dkg, darab should be db). Respond with a JSON array of objects. Each object must have an "item" key and a "quantity" key. Values should be in Hungarian, but transcribe medication names as-is. If a quantity is not specified, the "quantity" value should be an empty string. Example: [{"item": "tej", "quantity": "1 l"}, {"item": "Tensiomin", "quantity": "1 doboz"}, {"item": "sajt", "quantity": ""}, {"item": "tojás", "quantity": "10 db"}]. Do not include any other text or markdown formatting in your response.';
